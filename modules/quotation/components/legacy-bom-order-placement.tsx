...
import type { PaymentQuote } from "@/components/PaysharpCheckout";
import { formatCurrency } from "@/utils/format";

type Step = "payment" | "proof" | "processing" | "success";

export function LegacyBomOrderPlacement({
  bom,
  onClose,
  onSuccess,
  paymentQuote,
  ...
  checkoutKey: string;
  onResumePaysharp: (orderId: string) => void;
  bom: BomOrderData;
  onClose: () => void;
  onSuccess: () => void;
}) {
  ...
  const subtotal = paymentQuote.subtotalPaise / 100;
      ...
      : subtotal >= 1_000_000
        ...
        : subtotal >= 500_000
          ...
          : subtotal >= 1
            ? 2_500
            : 0;
  const tax = paymentQuote.taxPaise / 100;
  const total = paymentQuote.totalPaise / 100;
    ...
    try {
      const response = await fetch(`${MAIN_API_BASE_URL}/api/user/pi-generate`, {
        ...
        body: JSON.stringify({
          ...
          checkoutKey,
          expectedTotalPaise: paymentQuote.totalPaise,
